...
public interface RawDataReportRepository extends PagingAndSortingRepository<RawDataReport, String> {
    ...
    @Query(value = "SELECT ldr FROM RawDataReport ldr WHERE ((:sitesAll) is null or ldr.siteId IN (:siteids)) "
            + " AND ((:clusterAll) is null or ldr.cluster IN (:clusters)) AND (ldr.siteType) IN (:siteTypeID)  AND ldr.tranTime >= :startDate AND ldr.tranTime < :endDate")
    ...
    @Query(value = "SELECT ldr FROM RawDataReport ldr WHERE (ldr.siteType IN (:siteTypeID) AND ldr.tranTime >= :startDate AND ldr.tranTime < :endDate )")
    Page<RawDataReport> findAll(@Param("siteTypeID") Collection<String> siteTypes, Pageable pageable, @Param("startDate") String startDate, @Param("endDate") String endDate);
}
